...
async def _get_fun_settings(user_id: int) -> dict:
    ...
    Вынесено локально, чтобы избежать циклических импортов между
    handlers ↔ services ↔ ui.
    """
    try:
...
async def send_mention_notification(application: Application, user_id: int, mention_info: Dict[str, Any]) -> Optional[int]:
    try:
